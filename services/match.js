...
export class Match {
  ...
  addPlayer(name, order, flag) {
    var number = this.matchData.players.length + 1;
    var id = "player-" + number;
    var index = order - 1;
    var p = new player(name, order, index, id, flag);
    this.matchData.players.push(p);
  }

  playerIndex(playerid) {
    const player = this.matchData.players.map(e => e.id).indexOf(playerid);
    return this.matchData.players[player].index;
  }
  playerName(playerid) {
    const player = this.matchData.players.map(e => e.id).indexOf(playerid);
    return this.matchData.players[player].name;
  }

  getLeg() {
    var leg = 1;
    var scores = this.matchData.scores;
    if (!scores.length == 0) {
      var legs = [];
  ...
  getLegsPlayed(){
    var legs = [];
    this.matchData.scores.forEach(function(score) {
      if (legs.indexOf(score.leg) === -1) {
        legs.push(score.leg);
      }
    });
  ...
  getThrower(next) {
    ...
    var visit1 = this.getVisit(leg, this.matchData.players[0].id);
    var visit2 = this.getVisit(leg, this.matchData.players[1].id);
    var order1 = this.matchData.players[0].order;
    var order2 = this.matchData.players[1].order;
  ...
  getNewState(leg) {
    ...
    var wins = [0, 0];
    var remainings = [0, 0];
    var allScores = this.matchData.scores;
    var startCount = this.matchData.startCount;
    this.matchData.players.forEach(function(player) {
      var totalScore = 0;
      var lastTotal = 0;
  ...
  showDoubles(score, player, leg) {
    var show = false;
    var start = this.matchData.startCount;
  ...
  allPlayerStats() {
    ...
    var p1 = this.getStats(this.matchData.players[0].id)
    obj.push(p1)
    var p2 = this.getStats(this.matchData.players[1].id)
    obj.push(p2)
